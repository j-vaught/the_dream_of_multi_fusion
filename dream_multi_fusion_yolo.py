#!/usr/bin/env python3
"""
Multi-fusion pipeline: radar bounding boxes + YOLO-World self-labeling.
...
Produces three frame sequences (then encode to video with ffmpeg):
  ...
  3. vid3_yolo/      - per-frame vertical stack of YOLO-labeled crops (radar bbox + buffer)
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Multi-fusion: radar bbox + YOLO-World labeling pipeline")
    ...
    p.add_argument("--out-dir",
                    default="/Volumes/WorkDrive/10_10Data/dream_multi_fusion_yolo_output")
    p.add_argument("--buffer-px", type=int, default=150,
                    help="Padding in pixels around each radar bbox for the crop")
    p.add_argument("--model-weights", default="yolov8l-world.pt",
                    help="YOLO-World weights file")
    p.add_argument("--conf-thresh", type=float, default=0.25,
                    help="YOLO confidence threshold")
    p.add_argument("--classes", nargs="+", default=["boat", "buoy"],
                    help="Classes for YOLO-World open-vocabulary detection")
...
def load_yolo(weights: str, classes: List[str]):
    model = YOLOWorld(weights)
    model.set_classes(classes)
    return model


def run_yolo(model, image: Image.Image, conf: float) -> List[Dict]:
...
YOLO_DEFAULT_COLOR = (206, 211, 24)  # grass
...
def draw_yolo_on_crop(crop: Image.Image, dets: List[Dict], font) -> Image.Image:
    out = crop.copy()
    draw = ImageDraw.Draw(out)
    for d in dets:
        label = d["label"]
        color = YOLO_COLORS.get(label, YOLO_DEFAULT_COLOR)
...
def main() -> None:
    ...
    print(f"Loading YOLO-World ({args.model_weights}) with classes {args.classes}...")
    model = load_yolo(args.model_weights, args.classes)
    print("Model loaded.")
    ...
    for idx, rec in enumerate(frames):
        ...
        # --- Video 3: YOLO-labeled crops at full resolution ---
        crop_images = []
        crop_labels = []
        ...
        for det in detections:
            ...
            # Run YOLO-World on the full-resolution crop
            yolo_dets = run_yolo(model, crop, args.conf_thresh)

            crop_labeled = draw_yolo_on_crop(crop, yolo_dets, font_sm)
            crop_images.append(crop_labeled)

            n_boats = sum(1 for d in yolo_dets if d["label"] == "boat")
            n_buoys = sum(1 for d in yolo_dets if d["label"] == "buoy")
    ...
    videos = [
        ...
        ("vid3_yolo", "03_yolo_labeled_crops.mp4"),
    ]
